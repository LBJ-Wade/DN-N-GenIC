...
/* Eisenstein & Hu 
   Corrected for neutrinos.
   Returns power spectrum for CDM + Baryons + Neutrinos.
   Transfer Function tf_cbnu = T(k) * B(k) * D_cbnu(k,z)/D(z).
   Here B(k) is the corrective factor at the maximal free streaming scale.
   		D_cbnu(k,z) is the scale-dependent growth function due to massive neutrinos.
   		gf_cbnu(k,z) = D_cbnu(k,z) / D(z).
   
   (Himanish Ganjoo - March 2016)
*/
{
  double tf_cbnu = tk_eh(k)*b_k(k)*gf_cbnu(k,InitTime);
  
  return Norm * k * pow(tf_cbnu, 2);
}
...
double growth(double a)
{
  ...
  hubble_a = sqrt(Omega / (a * a * a) + (1 - Omega - OmegaLambda) / (a * a) + OmegaLambda * de_factor(a,DEParamChoice));
...
{
  return pow(a / (Omega + (1 - Omega - OmegaLambda) * a + OmegaLambda * a * a * a * de_factor(a,DEParamChoice)), 1.5);
}
...
double F_Omega(double a)
{
  ...
  omega_a = Omega / (Omega + a * (1 - Omega - OmegaLambda) + a * a * a * OmegaLambda * de_factor(a,DEParamChoice));
